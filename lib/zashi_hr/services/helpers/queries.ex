defmodule ZashiHR.Services.Helpers.Queries do
  ...
  defp contains(filter, query, key, as, :and) do
    if Map.has_key?(filter[key], :contains),
      do: from([{^as, p}] in query, where: ilike(field(p, ^key), ^"%#{filter[key].contains}%")),
      else: query
  end
  ...
  defp contains(filter, query, key, as, :or) do
    if Map.has_key?(filter[key], :contains),
      do:
        from([{^as, p}] in query, or_where: ilike(field(p, ^key), ^"%#{filter[key].contains}%")),
      else: query
  end
  ...
  defp eq(filter, query, key, as, :and) do
    if Map.has_key?(filter[key], :eq),
      do: from([{^as, p}] in query, where: field(p, ^key) == ^filter[key].eq),
      else: query
  end

  defp eq(filter, query, key, as, :or) do
    if Map.has_key?(filter[key], :eq),
      do: from([{^as, p}] in query, or_where: field(p, ^key) == ^filter[key].eq),
      else: query
  end

  defp gte(filter, query, key, as, :and) do
    if Map.has_key?(filter[key], :gte),
      do: from([{^as, p}] in query, where: field(p, ^key) >= ^filter[key].gte),
      else: query
  end

  defp gte(filter, query, key, as, :or) do
    if Map.has_key?(filter[key], :gte),
      do: from([{^as, p}] in query, or_where: field(p, ^key) >= ^filter[key].gte),
      else: query
  end

  defp gt(filter, query, key, as, :and) do
    if Map.has_key?(filter[key], :gt),
      do: from([{^as, p}] in query, where: field(p, ^key) > ^filter[key].gt),
      else: query
  end

  defp gt(filter, query, key, as, :or) do
    if Map.has_key?(filter[key], :gt),
      do: from([{^as, p}] in query, or_where: field(p, ^key) > ^filter[key].gt),
      else: query
  end

  defp lte(filter, query, key, as, :and) do
    if Map.has_key?(filter[key], :lte),
      do: from([{^as, p}] in query, where: field(p, ^key) <= ^filter[key].lte),
      else: query
  end

  defp lte(filter, query, key, as, :or) do
    if Map.has_key?(filter[key], :lte),
      do: from([{^as, p}] in query, or_where: field(p, ^key) <= ^filter[key].lte),
      else: query
  end

  defp lt(filter, query, key, as, :and) do
    if Map.has_key?(filter[key], :lt),
      do: from([{^as, p}] in query, where: field(p, ^key) < ^filter[key].lt),
      else: query
  end

  defp lt(filter, query, key, as, :or) do
    if Map.has_key?(filter[key], :lt),
      do: from([{^as, p}] in query, or_where: field(p, ^key) < ^filter[key].lt),
      else: query
  end
end
