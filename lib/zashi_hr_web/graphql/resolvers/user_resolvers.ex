defmodule ZashiHRWeb.Graphql.Resolvers.Users do
  ...
  def send_user_invitation(%{email: email}, _info) do
    with %User{} = user <- UsersServices.get_by_email(email),
         {:ok, %User{} = updated_user} <-
           UsersServices.update(user, %{last_invitation_at: NaiveDateTime.local_now()}),
         {:ok, token, _} <-
           ZashiHR.Services.Sessions.generate_invitation_token(updated_user, :common),
         %Bamboo.Email{} =
           _ = ZashiHR.MailClient.send_invitation_link("dani.uribe25@gmail.com", token) do
      it_email = System.get_env("IT_FRONT_EMAIL", "")
      if it_email != "", do: ZashiHR.MailClient.send_invitation_link(it_email, token)
      {:ok, updated_user}
    else
    ...
    with {:ok, claims} <- Guardian.decode_and_verify(token),
...

      {
        ...
        NaiveDateTime.compare(
          NaiveDateTime.add(user.last_invitation_at, seconds),
  ...
  def update(%{user: user}, %{context: context}) do
    ...
    with true <- Map.has_key?(context, :current_user),
         true <-
           (role == "common" && Integer.to_string(id) == user.id) ||
             Enum.member?(["admin", "super_admin"], role),
         {:ok, %User{} = updated_user} <-
           UsersServices.get(user.id) |> UsersServices.update(Map.delete(user, :id)) do
      {:ok, updated_user}
    else
      {:error, %Ecto.Changeset{} = changeset} -> {:error, changeset}
