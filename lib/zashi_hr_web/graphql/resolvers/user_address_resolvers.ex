defmodule ZashiHRWeb.Graphql.Resolvers.UserAddress do
  ...
  def create(%{user_address: attrs}, %{context: context}) do
    ...
    {:ok, user_address} <- UserAddressServices.create(new_attrs) do
      ...
      {:error, %Ecto.Changeset{} = changeset} -> {:error, changeset}
      %UserAddress{} = _user_address_found -> {:error, "There is an address already registered for this user, try updating it"}
      _ -> {:error, "Unknown"}
    end
  end

  def update(%{user_address: user_address}, %{context: context}) do
    with %UserAddress{} = address_to_update <- UserAddressServices.get_by_user_id(context.current_user.id),
      {:ok, %UserAddress{} = updated} <- UserAddressServices.update(address_to_update, user_address) do
        {:ok, updated}
    else
      {:error, %Ecto.Changeset{} = changeset} -> {:error, changeset}
